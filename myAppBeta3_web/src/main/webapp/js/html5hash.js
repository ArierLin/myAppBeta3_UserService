...
function selectFile(f) {
	(function() {
		...
		/*调用progressiveRead(file, work, done)方法开始*/
		progressiveRead(f, function(data, pos, file) {//第二个参数Work
			...
			// Update progress display更新进度显示
			var progress = Math.floor((pos / file.size) * 200);
			$("#readyProgressbar").css("float", "left");
			$("#readyProgressbar").css("width", progress);
			$("#readyPercentage").text("");
			$("#readyPercentage").text(Math.round(progress / 2) + "%");
			//计算耗时
			if (progress > lastprogress) {

				var took = ((new Date).getTime() - start) / 1000;
				$('#' + uid).html(
						file.name + '（' + bytes2si2(pos, file.size, 2)
								+ '）| 耗时: ' + digits(took, 2) + 's @ '
								+ bytes2si(pos / took, 2) + '/s | '
								+ new Date().getTime())
				lastprogress = progress;
			}
		}, function(file) {//第三个参数Done
			...
			$("#md5FileName").val(enabledAlgorithms[0].instance.finalize());
			//for (var j = 0; j < enabledAlgorithms.length ; j++) {
		...
		/*progressiveRead(file, work, done)方法调用结束*/
